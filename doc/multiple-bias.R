...
tab$latex <- sub("\\$$", "$$", tab$latex)
tab$bias <-  dplyr::case_when(
  tab$bias == "selection" & !grepl("Y\\*", tab$output) ~ "selection after outcome misclassification",
  tab$bias == "selection" & !grepl("A\\*", tab$output) ~ "selection after exposure misclassification",
  TRUE ~ tab$bias
)
tab$bias <- dplyr::case_when(
  ...
  tab$rare_exposure ~ paste0(tab$bias, " (rare exposure)")
)
